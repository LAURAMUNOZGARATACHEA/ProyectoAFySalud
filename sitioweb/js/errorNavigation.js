(function(global, factory) {
  ...
  function getMessage() {
    if (state.errors.length <= 0) {
      return LmForm.texts.doneMessage;
    }

    var msg="";
    if (state.errors.length === 1) {
      msg = LmForm.texts.oneError;
    }
    else{
      msg = LmForm.texts.multipleError;
    }
  ...
  function createNavigation(section) {
    ...
    nextButton.innerText = LmForm.texts.seeErrorsButton;
    ...
    doneButton.innerText = LmForm.texts.doneButton;
  ...
  function focusToNextError() {
    ...
    if (LmForm.doubleValidationFlag()) {
      ...
      if (sections.length > 1) {
        ...
        LmForm.jumpToPage(pagesIndex, true);
      }
    }
    if (sectionCollapse) {
